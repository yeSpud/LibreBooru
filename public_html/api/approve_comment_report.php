...
if ($report["status"] == "approved") {
    echo json_encode(["error" => $lang["report_already_approved"]]);
    http_response_code(400);
    exit();
}

$updateReportSql = "UPDATE comment_reports SET status = 'approved' WHERE report_id = ?";
$updateReportStmt = $conn->prepare($updateReportSql);
$updateReportStmt->bind_param("i", $_POST["id"]);
$updateReportStmt->execute();

$action = "approve_comment_report";
...
echo json_encode(["success" => true, "message" => $lang["report_approved"]]);
