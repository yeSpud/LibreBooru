...
$voteSql = "SELECT * FROM comment_votes WHERE comment_id = ? AND user_id = ? LIMIT 1";
...
if ($_vote) {
    if ($action == "remove") {
        $deleteSql = "DELETE FROM comment_votes WHERE comment_id = ? AND user_id = ?";
        ...
        $updateSql = "UPDATE comment_votes SET vote = ? WHERE comment_id = ? AND user_id = ?";
    ...
    if ($action != "remove") {
        $insertSql = "INSERT INTO comment_votes (comment_id, user_id, vote) VALUES (?, ?, ?)";
